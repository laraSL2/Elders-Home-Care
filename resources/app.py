...
selected = option_menu(
    menu_title=None,
    options=["Home", "Add New Elder", "Note Enhancement", "Plan Generation"],
...
# def split_care_plan(standardize_care_sub_plan):
#     sections = {
#         "Main Section": "",
...
#     for line in standardize_care_sub_plan.split('\n'):
#         line = line.strip()
#         if line.startswith('##'):
#             sections["Main Section"] = line.replace('##', '').strip()
...
#     st.title("Sub Plan Generation")

#     with st.form(key='care_plan_form'):
#         # Care note text area outside the columns but still inside the form
#         original_sub_plan = st.text_area("Enter the Sub plan:", height=500)

#         # Form submit button
#         col1, col2 = st.columns([4,1])

#         with col2:
#             submit_button = st.form_submit_button("Moderate", type="primary", use_container_width=True)
...
#     if submit_button:
#         if not original_sub_plan:
#             st.warning("Please enter the sub plan to standardize")
#         else:
#             st.session_state.standardize_care_sub_plan = standardize_care_plan(original_sub_plan)
#             st.session_state.standardize_care_sub_plan_copy = f"{st.session_state.standardize_care_sub_plan}"
#             st.session_state.button_clicked = True

#             # Split the care plan into sections
#             sections = split_care_plan(st.session_state.standardize_care_sub_plan)
#             # Display the care plan sections
#             display_care_plan(sections)
elif selected == "Plan Generation":
    st.title("Sub Plan Generation")
    ...
    if submit_button:
        if not original_sub_plan:
            ...
            # Display the output
            st.markdown(f"```\n{st.session_state.standardize_care_sub_plan}\n```")
                # st.write(st.session_state.standardize_care_sub_plan)
...
#     original_details = st.text_area("Enter the Sub Plan:", height=300)
...
#             st.warning("Please enter the sub plan")
...
#             refine_input = standardize_care_plan(text=original_details)
